...
class JB_Test_Fly_Plugin extends WP_UnitTestCase
{
    ...
    private static int $blog_2_id = 0;
     ...
     * Upload an image.
     */
    static function upload_image(): WP_Error|int
    {
    ...
    function test_get_image_size_string()
    {
        ...
        $this->assertSame(400, $image_size['size'][0], 'Wrong image width.');
        $this->assertSame(200, $image_size['size'][1], 'Wrong image height.');
    }
    ...
    function test_fly_get_image_size()
    {
        ...
        $this->assertSame(400, $image_size['size'][0], 'Wrong image width.');
        $this->assertSame(200, $image_size['size'][1], 'Wrong image height.');
        $this->assertSame(true, $image_size['crop'], 'Wrong crop.');
    }
    ...
    function test_get_all_image_sizes()
    {
        ...
        $this->assertNotTrue(empty($all_image_sizes), 'Image sizes empty.');

        $this->assertSame(200, $all_image_sizes['another_test_size']['size'][0], 'Wrong image width.');
        $this->assertSame(300, $all_image_sizes['another_test_size']['size'][1], 'Wrong image height.');
        $this->assertSame('left', $all_image_sizes['another_test_size']['crop'][0], 'Wrong crop X.');
        $this->assertSame('top', $all_image_sizes['another_test_size']['crop'][1], 'Wrong crop Y.');
    }
    ...
    function test_image_src()
    {
        ...
        $this->assertSame(400, $src['width'], 'Wrong width for SRC image with string name.');
        $this->assertSame(200, $src['height'], 'Wrong height for SRC image with string name.');

        $src = self::$_core->get_attachment_image_src(self::$_image_id, array(300, 100), true);
        $this->assertSame(300, $src['width'], 'Wrong width for SRC image with array values (hard).');
        $this->assertSame(100, $src['height'], 'Wrong height for SRC image with array values (hard).');

        $src = self::$_core->get_attachment_image_src(self::$_image_id, array(300, 100), false);
        $this->assertSame(178, $src['width'], 'Wrong width for SRC image with array values.');
        $this->assertSame(100, $src['height'], 'Wrong height for SRC image with array values.');
    }
    ...
    function test_image_html()
    {
        ...
        $this->assertTrue(str_starts_with($img, '<img'), 'IMG tag blank for image with string name.');

        $img = self::$_core->get_attachment_image(self::$_image_id, array(300, 100), true);
        $this->assertTrue(str_starts_with($img, '<img'), 'IMG tag blank for image with array values (hard).');

        $img = self::$_core->get_attachment_image(self::$_image_id, array(300, 100), false);
        $this->assertTrue(str_starts_with($img, '<img'), 'IMG tag blank for image with array values.');
    }
